...
exports.setupEvent = function(user, event, cb) {
	console.log('set up out look event for ' + user.name + ' : ' + event.title);
	console.log(event.start.toUTCString());
	hPostRequest('setup', user, event, cb);
};

exports.cancelEvent = function(user, event, cb) {
	console.log('Cancel out look event for ' + user.name + ' : ' + event.title);
	console.log(event.start.toUTCString());
...
function hPostRequest(method, user, event, cb){
	var post_data = querystring.stringify({
		...
		start_time: event.start.toUTCString(),
		end_time: event.end.toUTCString(),
		subject: event.title,
